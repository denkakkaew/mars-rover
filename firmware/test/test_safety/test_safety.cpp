// Host tests for lib/Safety — the failsafe contract in docs/protocol.md section 6.
//
...
/// A connected rover that was commanded at t=1000, evaluated at `now`.
...
// ---------------------------------------------------------------------------------
// millis() wraparound — the rover runs for weeks between reflashes
// ---------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------
// The Failsafe holder — event sequences
// ---------------------------------------------------------------------------------
...
void test_holder_survives_two_dropped_repeats(void) {
  // 150 ms cadence absorbs two consecutive losses (450 ms < 500 ms) — the margin the
...
void test_holder_mismatch_is_not_cleared_by_reconnecting(void) {
  // Safe mode clears on the next command; an incompatible peer does not. They are
  // deliberately different states (protocol.md 5).
  safety::Failsafe failsafe(kTimeout);
  failsafe.onConnect();
  failsafe.setPeerCompatible(false);
...
int main(int, char **) {
  ...
  RUN_TEST(test_holder_mismatch_is_not_cleared_by_reconnecting);
